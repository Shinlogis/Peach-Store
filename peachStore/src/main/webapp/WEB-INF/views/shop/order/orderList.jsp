										...
										<tbody>
											...
											<tr style="background-color: #f5f5f5; ">
												...
												<td colspan="2" style="font-weight: bold;">
													주문번호: <%=orderReceipt.getOrder_receipt_id()%> / 상태: <%=orderReceipt.getOrder_status()%>
												...
												<td style="margin-right: 20px;">
													<form method="post" action="/shop/order/cancle"
														...
														<input type="hidden" name="user.user_id"
															value="<%=user.getUser_id()%>"> <input
															type="hidden" name="order_receipt_id"
															value="<%=orderReceipt.getOrder_receipt_id()%>">
														<%
														if (orderReceipt.getOrder_status().equals("상품 준비 전") || orderReceipt.getOrder_status().equals("상품 준비 중")) {
														%>
														<button type="submit" class="btn btn-danger" style="padding: 2px 6px; font-size: 12px; line-height: 1.2;">주문취소</button>
														<%
														}
														%>
													</form>
